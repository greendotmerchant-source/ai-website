...
import { AMENITIES, REVIEWS, MAIN_PROPERTY_IMAGE, INTERIOR_IMAGE_1, INTERIOR_IMAGE_2, INTERIOR_IMAGE_3, INTERIOR_IMAGE_4, INTERIOR_IMAGE_5, INTERIOR_IMAGE_6, INTERIOR_IMAGE_7, INTERIOR_IMAGE_8, EVENT_IMAGE, GALLERY_IMAGES, LOGO_IMAGE, OWNER_PHONE, OWNER_EMAIL } from './constants';
import { Phone, MapPin, Star, ArrowUpRight, CheckCircle2, X, Maximize2, ChevronLeft, ChevronRight, Mail } from 'lucide-react';
          ...
          <div className="grid md:grid-cols-2 gap-32">
            <div>
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                <a href={`https://wa.me/${OWNER_PHONE}`} className="bg-gold text-charcoal px-12 py-6 rounded-sm font-bold uppercase tracking-[0.4em] text-[11px] hover:scale-105 transition-all text-center shadow-[0_10px_30px_rgba(212,175,55,0.3)] hover:bg-beige">WhatsApp Inquiry</a>
                <a href={`tel:+${OWNER_PHONE}`} className="border border-white/20 text-beige px-12 py-6 rounded-sm font-bold uppercase tracking-[0.4em] text-[11px] hover:bg-white/10 hover:border-gold transition-all text-center backdrop-blur-sm">Request Callback</a>
              </div>
            </div>
            ...
            <div className="bg-neutral-900/60 p-16 rounded-sm border border-gold/20 relative shadow-[0_30px_100px_rgba(0,0,0,0.5)]">
               {!isSubmitted ? (
                 ...
                 <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-700">
                    ...
                    <p className="text-beige/80 font-serif italic text-xl leading-relaxed max-w-sm mx-auto drop-shadow mb-12">
                      Thank you, {formState.name.split(' ')[0]}. Your request has been compiled. To finalize your priority reservation, please send it to us via your preferred secure channel.
                    </p>
                    
                    <div className="grid gap-4 w-full">
                      <a 
                        href={`https://wa.me/${OWNER_PHONE}?text=${encodeURIComponent(
                          `*Signature Villa Booking Inquiry*\n\n` +
                          `*Guest:* ${formState.name}\n` +
                          `*Contact:* ${formState.contact}\n` +
                          `*Residence:* ${formState.type}\n` +
                          `*Dates:* ${formState.checkIn} to ${formState.checkOut}\n` +
                          `*Guests:* ${formState.guests}\n` +
                          `*Requests:* ${formState.requests || "None"}\n\n` +
                          `Please confirm availability.`
                        )}`}
                        target="_blank"
                        rel="noreferrer"
                        className="flex items-center justify-center space-x-4 w-full bg-[#25D366] text-white py-6 rounded-sm font-bold uppercase tracking-[0.4em] text-[10px] hover:scale-[1.02] transition-all shadow-xl"
                      >
                        <Phone size={18} />
                      ...
                      <a 
                        href={`mailto:${OWNER_EMAIL}?subject=New Booking Inquiry - ${formState.name}&body=${encodeURIComponent(
                          `Signature Villa Booking Inquiry\n\n` +
                          `Guest: ${formState.name}\n` +
                          `Contact: ${formState.contact}\n` +
                          `Residence: ${formState.type}\n` +
                        ...
                        className="flex items-center justify-center space-x-4 w-full border border-white/20 text-beige py-6 rounded-sm font-bold uppercase tracking-[0.4em] text-[10px] hover:bg-white/10 transition-all font-bold"
                      >
                        <Mail size={18} />
                      ...
                      <button 
                        ...
                        className="mt-8 text-gold uppercase tracking-[0.4em] text-[9px] font-bold opacity-50 hover:opacity-100 transition-all radiant-glow"
                      >
                        Edit Application
                      </button>
                    </div>
      ...
      <footer className="bg-charcoal py-32 section-border">
        <div className="max-w-7xl mx-auto px-6 text-center flex flex-col items-center">
          ...
          <div className="flex justify-center space-x-16 mb-24">
            ...
            <a href={`mailto:${OWNER_EMAIL}`} className="text-beige/40 hover:text-gold transition-colors uppercase tracking-[0.4em] text-[10px] font-bold">Mail</a>
